...
def optimize(centerline, centerline_curvature, surface, df):
    ...
    for i, kappa_i in enumerate(centerline_curvature):
        ...
        if kappa_abs <= 1e-9:
            ...
            if f(v_max) >= 0:
                ...
                max_velocities[i] = brentq(f, v_min, v_max)
    ...
    ay_values = np.zeros(len(centerline))
    ax_values = np.zeros(len(centerline))
    for i in range(len(centerline) - 1):
        kappa_i = centerline_curvature[i]
        ay_values[i] = (forward_copy[i]**2)*abs(kappa_i)
        ax_values[i] = get_pts.query(ay_values[i], forward_copy[i], mode='accel')
    ay_values[-1] = ay_values[0]
    ax_values[-1] = ax_values[0]


    print('LAP TIME: ', lap_time)

    return forward_copy, lap_time, ay_values, ax_values
