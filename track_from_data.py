...
def heatmap_gps(data_path, target_lap_num, ax=None, vmin=None, vmax=None):
    # plot a heatmap of velocity around a track using real data
    ...
    centerline, curvature = smooth_gps_line(original_centerline, smoothing=5.0, num_points=len(subset))

    # get velocity values from subset (data logger records in km/h, convert to m/s)
    velocities = list(subset['Vehicle Speed'] / 3.6)
    print("LENGTH OF REAL VELOCITIES: ", len(velocities))
